...
e2e_pan=hg38.472-k151.e2e.gz  # ropebwt3 fa2kmer -k151 -w10 hs38.fa | ropebwt3 sw --all -t64 -N100 human472.fmd - 2> hg38.472-k151.e2e.log | gzip > hg38.472-k151.e2e.gz
e2e_ref=hg38.hg38-k151.e2e.gz # ropebwt3 fa2kmer -k151 -w10 hs38.fa | ropebwt3 sw --all -t64 -N100 GRCh38.fmd - 2> hg38.hg38-k151.e2e.log | gzip > hg38.hg38-k151.e2e.gz
gm_raw=hg38.genmap-K151E3.raw.bed.gz # genmap map -I hs38.genmap -O hg38.K151E3 -K 151 -E 3 --bedgraph -T 64
...
e2e_pan=chm13v2.472-k151.e2e.gz
e2e_ref=chm13v2.chm13v2-k151.e2e.gz
gm_raw=chm13.genmap-K151E3.raw.bed.gz
...
$(prefix).flt$(thres_ap).bed.gz:$(e2e_ref) $(e2e_pan)
	$(rb3tools) mapflt2 $(thres_ap) $(e2e_ref) $(e2e_pan) | bgzip > $@

$(prefix).flt$(thres_x).bed.gz:$(e2e_ref) $(e2e_pan)
	$(rb3tools) mapflt2 $(thres_x) $(e2e_ref) $(e2e_pan) | bgzip > $@

$(prefix).flt$(thres_y).bed.gz:$(e2e_ref) $(e2e_pan)
	$(rb3tools) mapflt2 $(thres_y) $(e2e_ref) $(e2e_pan) | bgzip > $@
...
$(prefix).fltgm.bed.gz:$(gm_raw)
	gzip -dc $< | awk '$$4<1{print $$1"\t"$$2"\t"$$3+150}' | bedtk merge | bgzip > $@

$(prefix)a.hard.bed.gz:$(prefix).flt$(thres_ap)ap.bed.gz $(prefix).flt$(thres_x)x.bed.gz $(prefix).flt$(thres_y)y.bed.gz $(prefix).fltgm.bed.gz $(prefix).ref-gap.bed.gz
	(gzip -dc $(prefix).flt{$(thres_ap)ap,$(thres_x)x,$(thres_y)y,gm}.bed.gz $(prefix).ref-gap.bed.gz) | awk '$$1~/^chr([0-9]+|[XY])$$/' | bedtk merge | awk '$$3-$$2>151' | bgzip > $@
...
$(prefix)b.easy.bed.gz:$(prefix)a.easy.bed.gz $(prefix).ref-dust.bed.gz
	gzip -dc $(prefix).ref-dust.bed.gz | awk '$$3-$$2>=19' | bedtk sub $(prefix)a.easy.bed.gz - | bgzip > $@
